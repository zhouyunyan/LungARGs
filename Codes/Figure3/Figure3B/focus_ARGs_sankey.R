...
#top5 ARGs: floR,tet(39), tet(L), tetQ and tet(D)
top5 <- c("floR","tet(39)","tet(L)","tetQ","tet(D)")
result <- result[which(result$ARG %in% top5),]
...
p <- sankeyNetwork(Links = link_list, Nodes = node_list,
                   ...
                   fontSize = 20,
                   sinksRight = FALSE,width = 1700,height = 600)

p
#save to .html
saveNetwork(p,"Top5_ARG_Taxa_Sankey.html")

library(webshot)
webshot("Top5_ARG_Taxa_Sankey.html","Top5_ARG_Taxa_Sankey.pdf",vwidth = 600,vheight=300)

...
path = ".\\MGE diversity"
input1 = "sample745_metadata_MGEalpha.xls"
input2 = "MGE_Type_83_abun.xls"
...
value <- c("Number", "Shannon", "Abun")
title.x <- NULL
title.y <- c("Richness (MGEs)","Shannon index (MGEs)","Abundance (MGEs)")
...
for (i in 1:length(value)) {
  filename <- paste(paste(group,"MGE",sep = "_"),value[i],sep = ".")
  #tif
...
Beta <- p.beta(distance.data,"Group","dt.distance",NULL,"Beta Diversity (MGEs)",label, my_comparisons)

p.save <- function(group,width,height){
  filename <- paste(paste(group,"MGE",sep = "_"),"Beta",sep = ".")
  #tif
...
p.save <- function(group,width,height){
  filename <- paste(paste(group,"MGE",sep = "_"),"PCoA",sep = ".")
  #tif
